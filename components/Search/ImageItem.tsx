...
const ImageItem = ({ data }: IImageItem) => {
    ...
    const onClick = () => {
        dispatch(addRecentviewList({ url: data.url, id: data.product_idx }));
    }

    const onImgClick = () => {
        router.push(`/product/${data.product_idx}`);
    }
    ...
    return <li onClick={onClick} className="w-[340px]">
        <div className="relative w-full h-[435px] bg-slate-500 mb-4">
            <Image onClick={onImgClick} layout="fill" objectFit='cover' src={data.url} />
        ...
        <Link href={`/product/${data.product_idx}`}>
            <a>
                <div className="space-y-3">
                    <h1 className="text-xl">{data.brand_name ? `[${data.brand_name}]` : null}{data.name}</h1>
                    {data.discount ?
                        <div>
                            <div className="text-lg font-semibold">
                                <span className="text-orange-500 mr-2">{data.discount}%</span>
                                <span>{data.discount_price}원</span>
                            </div>
                            <div className="text-base text-gray-400 font-semibold line-through">{data.price}원</div>
                        </div>
                        :
                        <div className="text-lg font-semibold">{data.price}원</div>
                    }
                    <p className="text-sm text-gray-400 font-semibold">{data.subname}</p>
                    {
                        data.is_kurlyonly ?
